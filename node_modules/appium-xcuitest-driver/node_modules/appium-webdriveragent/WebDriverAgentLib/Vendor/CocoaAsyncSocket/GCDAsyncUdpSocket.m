...
- (const struct sockaddr *)extracted:(NSData *)localAddr6 {
  return (const struct sockaddr *)[localAddr6 bytes];
}
...
- (BOOL)bindToAddress:(NSData *)localAddr error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
      ...
      int status = bind(self->socket6FD, [self extracted:localAddr6], (socklen_t)[localAddr6 length]);
			if (status == -1)
			{
				[self closeSockets];
